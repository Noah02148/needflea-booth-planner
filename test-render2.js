...
(async () => {
  ...
  // STEP 1: Read model space — compute bounds from LINE/POLY ONLY (before block expansion)
  const msEntities = [];
  const bxs = [], bys = [];
  ...
  // STEP 2: Compute bounds from model space geometry only
  ...
  console.log(`Bounds: X[${bx0.toFixed(0)},${bx1.toFixed(0)}] Y[${by0.toFixed(0)},${by1.toFixed(0)}]`);

  // STEP 3: Expand blocks and filter — only process INSERTs within bounds
  const items = [];
  msEntities.forEach(item => {
    if (item.t === 'I') {
      if (inB(item.x, item.y)) expandInsert(item, blockDefs, items, 0);
    } else {
      items.push(item);
    }
  });

  // STEP 4: Filter all items to bounds
  ...
  console.log(`Items: ${items.length}, filtered: ${filtered.length}`);

  // STEP 5: SVG
  const W=1600, H=1000, bw=bx1-bx0, bh=by1-by0;
  const sc=Math.min((W-80)/bw,(H-80)/bh);
  const ox=40-bx0*sc, oy=40+by1*sc;
  const sx=x=>(x*sc+ox).toFixed(1), sy=y=>(-y*sc+oy).toFixed(1);

  let svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" style="background:#1e1e1e">\n`;
  filtered.forEach(it => {
...
function extractEntity(lib, e, layerColors, layerLT) {
  ...
  try{
    if(e.eType==lib.DRW_ETYPE.LINE){const bp=e.basePoint,sp=e.secPoint;return{t:'L',x1:bp.x,y1:bp.y,x2:sp.x,y2:sp.y,c:hex,lt};}
    if(e.eType==lib.DRW_ETYPE.LWPOLYLINE){const verts=e.getVertexList();const pts=[];for(let j=0;j<verts.size();j++)pts.push([verts.get(j).x,verts.get(j).y]);if(pts.length>=2)return{t:'P',pts,closed:(e.flags&1)===1,c:hex,lt};}
    if(e.eType==lib.DRW_ETYPE.CIRCLE){const bp=e.basePoint;return{t:'C',cx:bp.x,cy:bp.y,r:e.radius,c:hex};}
    if(e.eType==lib.DRW_ETYPE.ARC){const bp=e.basePoint;return{t:'A',cx:bp.x,cy:bp.y,r:e.radius,sa:e.startAngle,ea:e.endAngle,c:hex};}
    if(e.eType==lib.DRW_ETYPE.TEXT||e.eType==lib.DRW_ETYPE.MTEXT){
      ...
      if(text)return{t:'T',x:bp.x,y:bp.y,text,h:e.height||1,c:hex};
    }
    if(e.eType==lib.DRW_ETYPE.INSERT){const bp=e.basePoint;return{t:'I',block:e.name||'',x:bp.x,y:bp.y,sx:e.xScale||1,sy:e.yScale||1,angle:e.angle||0,c:hex};}
  }catch(err){}
  return null;
}
...
function expandInsert(ins, blockDefs, output, depth) {
  ...
  block.forEach(e=>{
    if(e.t==='L'){const[x1,y1]=tf(e.x1,e.y1),[x2,y2]=tf(e.x2,e.y2);output.push({t:'L',x1,y1,x2,y2,c:e.c,lt:e.lt});}
    if(e.t==='P'){output.push({t:'P',pts:e.pts.map(p=>tf(p[0],p[1])),closed:e.closed,c:e.c,lt:e.lt});}
    if(e.t==='C'){const[cx,cy]=tf(e.cx,e.cy);output.push({t:'C',cx,cy,r:e.r*Math.abs(ins.sx),c:e.c});}
    if(e.t==='A'){const[cx,cy]=tf(e.cx,e.cy);output.push({t:'A',cx,cy,r:e.r*Math.abs(ins.sx),sa:e.sa+ins.angle,ea:e.ea+ins.angle,c:e.c});}
    if(e.t==='T'){const[x,y]=tf(e.x,e.y);output.push({t:'T',x,y,text:e.text,h:e.h*Math.abs(ins.sy),c:e.c});}
    if(e.t==='I'){const[x,y]=tf(e.x,e.y);expandInsert({...e,x,y,sx:e.sx*ins.sx,sy:e.sy*ins.sy,angle:e.angle+ins.angle},blockDefs,output,depth+1);}
  });
}
